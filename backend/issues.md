...
| 4 | **Rate-limit auth endpointokon** – `express-rate-limit` a `/login`, `/register`, `/profile/verify-current-password` útvonalakra (brute-force védelem). | [backend/api/routes/auth.js](backend/api/routes/auth.js) | ☐ |
